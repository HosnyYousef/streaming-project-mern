...
export const subscribe = async (req, res, next) => {
  try {
    ...
    await User.findByIdAndUpdate(me, { $addToSet: { subscribedUsers: targetId } });
...
export const like = async (req, res, next) => {
  ...
  const videoId = req.params.id;   // MUST be :id to match the route
  ...
  try {
    ...
    return res.status(200).json({ message: "liked", likes: updated.likes }); // return proof
  } catch (err) {
    return next(err);
  }
};
